...
func main() {
	...
	uniqueVindex, ok := primaryVindex.Vindex.(vindexes.Unique)
	if !ok {
	...
	err = sourceReplicaQs.StreamExecute(ctx, &sourceReplicaTarget, selectSql, make(map[string]*query.BindVariable), &executeOptions, func(result *sqltypes.Result) error {
		...
		for _, row := range result.Rows {
			...
			ksids, err := uniqueVindex.Map(nil, []sqltypes.Value{primaryVindexVal})
			if err != nil {
				return err
			}
			if ksids[0].Range != nil || ksids[0].ID == nil {
				return fmt.Errorf("could not map %v to a keyspace id", primaryVindexVal)
			}
			if !key.KeyRangeContains(destShard.KeyRange, ksids[0].ID) {
				continue
			}
